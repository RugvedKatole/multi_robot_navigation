...
def RVO_update(X, V_des, V_current, ws_model):
    ...
    for i in range(len(X)):         # X is the current position of robot in X Y #here vector of 4
        ...
        for j in range(len(X)):
            if i!=j:
                vB = [V_current[j][0], V_current[j][1]]  #current velocity of obstacle bot 
                pB = [X[j][0], X[j][1]] #current position of obstacle bot
                # use RVO
                ...
                theta_BAort = asin(2*ROB_RAD/dist_BA)   # angle between tanget and central line 
                theta_ort_left = theta_BA+theta_BAort   # aplha/2 wala angle 
                bound_left = [cos(theta_ort_left), sin(theta_ort_left)] #left bound 
                theta_ort_right = theta_BA-theta_BAort
                bound_right = [cos(theta_ort_right), sin(theta_ort_right)] #right bound
                # use HRVO
                dist_dif = distance([0.5*(vB[0]-vA[0]),0.5*(vB[1]-vA[1])],[0,0])
                transl_vB_vA = [pA[0]+vB[0]+cos(theta_ort_left)*dist_dif, pA[1]+vB[1]+sin(theta_ort_left)*dist_dif] #apex of cone
                RVO_BA = [transl_vB_vA, bound_left, bound_right, dist_BA, 2*ROB_RAD]  # list of all the collision cones of all bots
        ...
        #     RVO_BA_all.append(RVO_BA)
        # RVO_BA is collection of all the collision cones
...
def intersect(pA, vA, RVO_BA_all):
    # check if the desired velocity is suitable or not
    ...
    for theta in numpy.arange(0, 2*PI, 0.05):
        for rad in numpy.arange(0.02, norm_v+0.02, norm_v/10.0):
            ...
            for RVO_BA in RVO_BA_all:
                p_0 = RVO_BA[0]   #apex point 
                left = RVO_BA[1] # left side vector
                right = RVO_BA[2] # right side vector
                dif = [new_v[0]+pA[0]-p_0[0], new_v[1]+pA[1]-p_0[1]]  
                theta_dif = atan2(dif[1], dif[0])
                theta_right = atan2(right[1], right[0])
                theta_left = atan2(left[1], left[0])
                if in_between(theta_right, theta_dif, theta_left):   #returns true if theta diff is between right and left theta
                    suit = False
                    break
            if suit:
    ...
    # choosing among the suitable velocities        
    if suitable_V:
        vA_post = min(suitable_V, key = lambda v: distance(v, vA)) # suitable velocity nearest to current V
        new_v = vA_post[:]
        for RVO_BA in RVO_BA_all:
            p_0 = RVO_BA[0]
            left = RVO_BA[1]
