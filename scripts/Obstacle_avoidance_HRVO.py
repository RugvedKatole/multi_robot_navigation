...
class RobotHRVO(object):
    def __init__(self, no_of_robots):
        ...
        self.PID = [PID_control("bot_%i" %i,publisher_name="/tb3_%i/cmd_vel" %i,odom_name= "/tb3_%i/odom" %i) for i in range(self.total_bots)]
        ...
        self.timer = rospy.Timer(rospy.Duration(0.02),self.hrvo)
    ...
    def hrvo(self, event):
        ...
        for i in range(self.total_bots):
            ...
            self.PID[i].Velocity_tracking_law(self.velocity[i][0],self.velocity[i][1])
